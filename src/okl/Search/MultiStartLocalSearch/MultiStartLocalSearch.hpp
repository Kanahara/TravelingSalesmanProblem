...
namespace okl {
	/*!
	 *  @brief      多スタート局所探索関数クラス
	 *  @details    多スタート局所探索関数クラス
	 *				定義された局所探索と初期解生成により多スタート局所探索を行う
	...
	template<
		...
		class MultiStartLocalSearch : public Search<typename __LocalSearch::solution_type, __EndCondition>
	{
	public:
		/*!
		 * @brief				コンストラクタ
		 */
		MultiStartLocalSearch() = default;

		/*!
		 * @brief				デストラクタ
		 */
		virtual ~MultiStartLocalSearch() = default;
		 ...
		 */
		static const std::string info() { return "多スタート局所探索法<" + __EndCondition::info() + ">(" + __LocalSearch::info() + ")(" + __Start::info() + ")(" + __ReStart::info() + ")"; }
		...
		inline void seed(const seed_type __seed) override
		{
			...
			this->restart_.seed(this->random_device_.uniform_int(std::numeric_limits < std::seed_seq::result_type >::min(), std::numeric_limits < std::seed_seq::result_type >::max()));
		}
		...
		inline void operator()(graph_type& __graph, __SearchInfo& __search_info, solution_type& __solution)
		{
			...
			while (true)
			{
				...
				if (this->isEnd(__graph, __search_info, __solution)) {
					...
					this->ReStart(__graph, __search_info, __solution);
				}
			}
	...
	protected:
		...
		inline void LocalSearch(graph_type& __graph, __SearchInfo& __search_info, solution_type& __solution)
		{
			...
			//std::cout << "--局所探索後-----" + __LocalSearch::info() << std::endl;
		 ...
		 * @param[out]		__SearchInfo 探索情報クラス
		 * @param[out]		solution_type	 解クラス
		 * @detail			2回目以降の初期解生成を行う
		 * 					- graph_typeはテンプレート__LocalSearchのsolution_typeのgraph_typeによって定義される
		 * 					- __SearchInfoはテンプレート__SearchInfoによって定義される
		 * 					- solution_typeはテンプレート__LocalSearchのsolution_typeによって定義される
		 */
		template<class __SearchInfo>
		inline void ReStart(graph_type& __graph, __SearchInfo& __search_info, solution_type& __solution)
		{
			this->restart_(__graph, __search_info, __solution, this->initial_solution_, this->best_solution_);
			//std::cout << "--再初期解生成後-----" + __Start::info() << std::endl;
			//std::cout << __solution << std::endl;
			//std::cout << "-------" << std::endl;
		}
