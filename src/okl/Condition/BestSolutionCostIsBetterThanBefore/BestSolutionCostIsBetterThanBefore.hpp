...
namespace okl {
	/*!
	 *  @brief      ベストコストが一つ前の状態より良いか判定する
	 *  @details    ベストコストが一つ前の状態より良いか判定する基底クラス
	 *				大きい場合はTrue
	 *				- __Cost				コスト数値型
	...
	template<
		...
		class BestSolutionCostIsBetterThanBefore : public Condition
	{
	public:
		/*!
		 * @brief				コンストラクタ
		 */
		BestSolutionCostIsBetterThanBefore() : before_cost_(this->undefine_cost()) {};

		/*!
		 * @brief				デストラクタ
		 */
		virtual ~BestSolutionCostIsBetterThanBefore() = default;
		 ...
		 */
		static const std::string info() { return "前よりベスト解が良かったら"; }
		...
		{
			cost_type _best_cost = 0;
			if (cost_comparison_function()(__solution.cost(), __best_solution.cost())) {
				_best_cost = __best_solution.cost();
			}
			else {
				_best_cost = __solution.cost();
			}

			if (this->before_cost_ == this->undefine_cost() || cost_comparison_function()(this->before_cost_, _best_cost)) {
