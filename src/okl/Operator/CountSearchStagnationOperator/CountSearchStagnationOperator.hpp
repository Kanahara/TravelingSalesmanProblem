...
namespace okl {
	/*!
	 *  @brief      探索停滞回数カウント関数クラス
	 *  @details    探索停滞回数を+1をする関数クラス
	 */
	class CountSearchStagnationOperator : public Operator
	{
	public:
		/*!
		 * @brief				コンストラクタ
		 */
		CountSearchStagnationOperator() = default;

		/*!
		 * @brief				デストラクタ
		 */
		virtual ~CountSearchStagnationOperator() = default;
		 ...
		 */
		static const std::string info() { return "探索停滞カウント"; }
		...
		{
			__search_info.countSearchStagnation();
		}
		template<class __SearchInfo, class __Solution>
