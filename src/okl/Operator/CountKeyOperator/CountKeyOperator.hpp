...
namespace okl {
	...
	class CountKeyOperator : public Operator
	{
	public:
		/*!
		 * @brief				コンストラクタ
		 */
		CountKeyOperator() = default;

		/*!
		 * @brief				デストラクタ
		 */
		virtual ~CountKeyOperator() = default;
		 ...
		 */
		static const std::string info() { return "カウント:" + std::to_string(__KeyIndex); }
		...
		{
			__search_info.countKey(__KeyIndex);
		}
		template<class __SearchInfo, class __Solution>
