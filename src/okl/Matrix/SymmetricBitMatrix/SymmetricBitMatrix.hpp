...
namespace okl {
	...
	template<
		...
		class SymmetricBitMatrix : public Matrix<__Value, __Key> {
		public:
			/*!
			 * @brief				コンストラクタ
			 */
			SymmetricBitMatrix() = default;

			/*!
			 * @brief				デストラクタ
			 */
			virtual ~SymmetricBitMatrix() = default;
			...
			inline void resize(const size_type __size) override final
			{
				...
				this->alpha_ = static_cast<key_type>(2 * this->size_ - 1);
				this->container_.reserve(static_cast<typename __Container::size_type>(this->size_ * (this->size_ + 1) / 2));
			}
